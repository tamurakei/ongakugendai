jQuery(document).ready(function($) {
    'use strict';
    var this_obj = mega_store_companion_install;

    $(document).on('click', '.mega-store-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.addClass('button-primary mega-store-plugin-activate');
    ...
    $(document).on('click', '.mega-store-plugin-activate', function(event) {
        event.preventDefault();
        var button = $(this);
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'mega_store_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '#companion-install-dismiss', function(event) {
        ...
        $.ajax({
            url: this_obj.ajax_url,
            type: 'POST',
            data: {action: 'mega_store_hide_customizer_companion_notice'},
            success:function(data) {
                $container.remove();
            }
        });
