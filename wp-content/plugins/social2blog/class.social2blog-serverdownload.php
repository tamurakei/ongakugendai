...
class Social2blog_Serverdownload {
	...
	public function get_posts() {
	  ...
	  if ($social2blog_state == true) {
		...
		$social2blog_lastidpost =$wpdb->get_var("
				SELECT MAX(`meta_value`) FROM `wp_postmeta`
				WHERE
	...
	public function createPost($post_info) {
		...
		$querystr = "SELECT `ID` FROM `wp_posts` WHERE `ID` IN (SELECT `post_id` FROM wp_postmeta WHERE `meta_value` = '{$post_info->post_id}')";
...
/*						if( empty( $post_info->category ) ){
							wp_delete_post($lastid);
							throw new Social2blog_Exception( __('Download errato, nessun tag.', 'social2blog-text') );
						}

						$categorie = explode( "," , $post_info->category);
						for ($i = 0 ; $i < count($categorie) ; $i++){
							$cat_exist = get_cat_ID( $categorie[$i]);
							// Controllo se la categoria in ingresso esiste, altrimenti non inserisco il post
							if( $cat_exist == "0" ){
								wp_delete_post($lastid);
								throw new Social2blog_Exception( __('Tag "'.$categorie[$i].'" errata, categoria mancante.', 'social2blog-text') );
							}
						}
*/						$categorie = explode( "," , $post_info->category);
	 ...
	 */
		...
		global $wpdb;
		$querystr = "SELECT wp_posts.ID FROM wp_posts INNER JOIN wp_postmeta ON ( wp_posts.ID = wp_postmeta.post_id ) WHERE wp_postmeta.meta_key = 'facebook_event_id' AND wp_postmeta.meta_value = '{$post_info->event_id}' AND (wp_posts.post_status = 'publish' OR wp_posts.post_status = 'draft' OR wp_posts.post_status =  'trash') GROUP BY wp_posts.ID ORDER BY wp_posts.post_date DESC LIMIT 0, 1";
		$pageposts = $wpdb->get_results($querystr, OBJECT);
		...
		if ( empty($event_id) ){
			...
			global $wpdb;
			$querystr = "SELECT wp_posts.ID FROM wp_posts INNER JOIN wp_postmeta ON ( wp_posts.ID = wp_postmeta.post_id ) WHERE wp_postmeta.meta_key = 'location_event_id' AND wp_postmeta.meta_value = '{$key_identifier}' AND wp_posts.post_status = 'publish' GROUP BY wp_posts.ID ORDER BY wp_posts.post_date DESC LIMIT 0, 1";
...
// 		$hour = explode( "+", $info_data[1])[0];
	...
	public function updateServerInfo(){
		global $wpdb;
		$querystr = "SELECT `option_name`,`option_value` FROM `wp_options` WHERE `option_name` LIKE 'social2blog%'";
