...
class Social2blog_Facebook {
	...
	public function saveAccessToken($access_token, $api_key_xkoll) {
		...
		$graph_url_pages = "http://splws.xkoll.com/api/access_token/get_access_token.php?idcode=".$icode."&api_key=".$api_key_xkoll."&fb_exchange_token=".$access_token;


		$output = Social2blog_Http::requestHttp($graph_url_pages);
		$ssa_ = str_replace("access_token=", "", $output);

		update_option('social2blog_fb_access_token', $ssa_);
		$this->access_token = $ssa_;
	}
	...
	public function removeAccessToken(){
		$del = delete_option('social2blog_fb_access_token');
	...
	public function capturePageAdmin() {
		$graph_url_pages = "https://graph.facebook.com/v2.5/me/accounts?access_token=" . $this->access_token;
		...
		curl_close ( $ch );
		$pages = $output; // get all pages information from above url.
		                  // error_log ( print_r ( json_decode ( $pages, true ), TRUE ) );
	...
	public function  getPageToken($id_page){
		...
		$graph_url_pages = "https://graph.facebook.com/v2.5/$id_page?fields=access_token&access_token=" . $this->access_token;
	...
	/**
	 * Se è memorizzato l'access_token di facebook restituisce vero
	 */
